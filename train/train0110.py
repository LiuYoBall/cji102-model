...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        # 使用 binary_cross_entropy_with_logits 計算原始 BCE
        ...
        if self.alpha is not None:
            # 簡單的廣播機制處理多標籤權重
            # alpha 形狀應為 [1, num_classes] 或 [num_classes]
...
def train_one_epoch(model, loader, optimizer, device, scaler, grad_accum_steps: int, model_ema=None):
    ...
    # [V4.0 核心修正] Focal Loss
    # Normal/DR=1 (數量多，讓 Focal Loss 自己降權)
    # Glaucoma/Cataract=2 (稍微加強關注即可)
    alpha_weights = torch.tensor([1.0, 1.0, 2.0, 2.0]).to(device)
    
    # gamma=2.0 是解決難易樣本失衡的黃金標準
    ...
    for step, (x, y) in enumerate(loader, start=1):
        ...
        with torch.amp.autocast(device_type="cuda", enabled=(device.type == "cuda")):
            logits = model(x)
            loss = criterion(logits, y_smooth) / grad_accum_steps
...
def main():
    ...
    best_auc = -1.0
    print("Start Training V4.0 (Focal Loss + Balanced Thresholds)...")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(f"   🔥 Suggested Thresholds: {best_thr}") 
