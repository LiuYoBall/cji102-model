...
def crop_image_from_gray(img_arr, tol=7):
    ...
    if img_arr.ndim == 2:
        ...
        if (check_shape == 0):
            ...
            rows = mask.any(1)  
            cols = mask.any(0)  
            return img_arr[rows][:, cols] # 先切列，再切行
    return img_arr
...
def load_retfound_weights_into_vit(vit: nn.Module, ckpt_path: str):
    ...
    # === [關鍵修正] 位置編碼插值 (Positional Embedding Interpolation) ===
    if 'pos_embed' in new_sd:
        # 取得 checkpoint 中的 pos_embed 形狀，例如 [1, 197, 1024]
        ckpt_pos_embed = new_sd['pos_embed']
        # 取得當前模型需要的 pos_embed 形狀，例如 [1, 577, 1024]
        model_pos_embed = vit.pos_embed
        
        if ckpt_pos_embed.shape != model_pos_embed.shape:
            print(f"⚠️ Resizing pos_embed: {ckpt_pos_embed.shape} -> {model_pos_embed.shape}")
            
            # 1. 取出 CLS token (第0個) 與 Grid tokens (第1個之後)
            cls_pos_embed = ckpt_pos_embed[:, 0:1, :]
            grid_pos_embed = ckpt_pos_embed[:, 1:, :]
            
            # 2. 計算舊的網格大小 (例如 sqrt(196) = 14)
            n_tokens_old = grid_pos_embed.shape[1]
            gs_old = int(math.sqrt(n_tokens_old))
            
            # 3. 計算新的網格大小 (例如 sqrt(576) = 24)
            n_tokens_new = model_pos_embed.shape[1] - 1
            gs_new = int(math.sqrt(n_tokens_new))
            
            # 4. 重塑形狀以進行插值: [B, L, C] -> [B, C, H, W]
            # permute: (1, 196, 1024) -> (1, 1024, 196)
            # reshape: -> (1, 1024, 14, 14)
            grid_pos_embed = grid_pos_embed.permute(0, 2, 1).reshape(1, -1, gs_old, gs_old)
            
            # 5. 執行雙三次插值 (Bicubic Interpolation) 放大
            grid_pos_embed = F.interpolate(
                grid_pos_embed, size=(gs_new, gs_new), mode='bicubic', align_corners=False
            )
            
            # 6. 轉回原本格式: [B, C, H, W] -> [B, L, C]
            grid_pos_embed = grid_pos_embed.flatten(2).permute(0, 2, 1)
            
            # 7. 拼接回 CLS token
            new_pos_embed = torch.cat((cls_pos_embed, grid_pos_embed), dim=1)
            
            # 8. 更新 state_dict
            new_sd['pos_embed'] = new_pos_embed
    # ================================================================
...
def build_model(num_labels: int, ckpt_path: str = "", freeze_backbone: bool = False, 
                backbone_name: str = "vit_large_patch16_224", img_size: int = 224):
    """
    修改點：加入了 img_size 參數，並傳遞給 timm.create_model
    """
    print(f"Creating model: {backbone_name} with img_size={img_size}...")
    
    # 關鍵修改：加入 img_size=img_size
    # 這會觸發 timm 自動將 224 的位置編碼插值(Resize)成 384，解決尺寸不合問題
    vit = timm.create_model(backbone_name, pretrained=False, num_classes=0, img_size=img_size)
...
def train_one_epoch(model, loader, optimizer, device, scaler, grad_accum_steps: int, model_ema=None):
    model.train()

    # 對應順序: [y_normal, y_dr, y_glaucoma, y_cataract]
    #  labels 順序須一致 
    class_weights = torch.tensor([1.1, 1.4, 2.3, 6]).to(device)
    ...
    for step, (x, y) in enumerate(loader, start=1):
        x = x.to(device, non_blocking=True)
        y = y.to(device)
        y_smooth = y * (1 - 0.1) + 0.5 * 0.1

        with torch.amp.autocast(device_type="cuda", enabled=(device.type == "cuda")):
            logits = model(x)
            loss = loss_fn(logits, y_smooth) / grad_accum_steps
...
def main():
    ...
    model = build_model(num_labels=len(label_cols), ckpt_path=ckpt_path, 
                        freeze_backbone=args.freeze_backbone, backbone_name=args.backbone, 
                        img_size=args.img_size).to(device)

    # 初始化 EMA，decay=0.9999 是標準值
    ...
    # === 接續訓練的完整載入邏輯 ===
    if ckpt_path:
        print(f"Attempting to resume training state from {ckpt_path}...")
        try:
            # 讀取 checkpoint
            ...
            if "optimizer" in checkpoint:
                ...
                print("⚠️ Warning: No optimizer state in checkpoint (Normal for first resume).")
            ...
            # 3. 嘗試載入 Epoch (可選：讓顯示的 Epoch 數字接續)
            ...
            print(f"⚠️ Resume failed (only weights loaded): {e}")
            start_epoch = 1
    else:
        start_epoch = 1
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        t0 = time.time()
        train_loss = train_one_epoch(model, train_loader, optimizer, device, scaler, args.grad_accum, model_ema=model_ema)
        val_loss, val_auc, label_aucs = evaluate(model_ema.module, val_loader, device)
        dt = time.time() - t0
