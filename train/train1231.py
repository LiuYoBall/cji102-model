...
class MultiLabelFundusDataset(Dataset):
    def __init__(self, csv_path: str, data_root: str, img_size: int = 224, 
        ...
        for idx, rel_path in enumerate(all_paths):
            ...
            if full_path is None: # 找不到圖
                missing_count += 1
                if missing_policy == "raise":
                    raise FileNotFoundError(f"Image not found: {rel_path} in {data_root}")
                elif missing_policy == "skip":
                    continue # 跳過，不加入 valid_indices
                elif missing_policy == "black":
                    self.valid_indices.append((idx, None)) # 標記為 None，getitem 時產生黑圖
            else:
                self.valid_indices.append((idx, full_path)) # 存入 (原始index, 解析後的絕對路徑)

        # 如果 policy 是 skip，更新 DataFrame 顯示實際數量
    ...
    def resolve_image_path(self, rel_path: str):
        """
        智慧路徑解析：嘗試對齊 data_root 與 rel_path
        """
        # 1. 直接拼接
        p1 = os.path.join(self.data_root, rel_path)
        if os.path.exists(p1): return p1
        
        # 2. 嘗試去除開頭的 'images/' (因有時 CSV 包含 images/ 但 data_root 也指向 images)
        ...
        # 3. 嘗試加上 'images/' (因有時 data_root 指向上一層)
...
def main():
    ...
    # 1. 定義預設變數
    default_out_dir = "/content/drive/MyDrive/Model_outputs" # 依您的截圖修改
    default_drive_root = "/content/drive/MyDrive/fundus-retfound-data" # 這裡不加 cfp
    ...
    ap.add_argument("--out_dir", type=str, default=default_out_dir) # 修正：直接用變數
    ...
    ap.add_argument("--lr", type=float, default=1e-5) # Fine-tune 小 LR
    ...
    if not args.data_root:
        if os.path.exists(local_root):
            ...
            # 如果本地沒有，則回退到 Drive 的 images 資料夾 (無 cfp 層)
